...
for i in range(puzz_counter,1000000):

    env.base = puzzles[puzz_counter][0]
    env.sol = puzzles[puzz_counter][1]
    env.verbose = False
    puzz_counter += 1


    print('Starting environment on puzzle %d: ' % i)
    state = env.reset()
    #env.render()
    
    ns = len(helper.get_zero_elements(env.base))
    ...
    while True:
        ...
        if (action >= 9):
            movement_repeated_count += 1
            if (movement_repeated_count > 10):
                #print('Model got in an infinite movement loop! Breaking!')
                got_stuck = True
        else:
            movement_repeated_count = 0
        
        # print('Environment after action, puzzle %d, tot_reward %0.2f:' % (i,test_reward_for_ep))
        # env.render()
        ...
        test_num_iters += 1
        if (done or got_stuck):
            ne = len(helper.get_zero_elements(env.grid))
            
            perc_completed = 100*(ns-ne)/ns
            test_results.append({'puzz': i, 'perc_completed': perc_completed})
            
            # print('Num empty cells start: %s' % str(num_empty_starting_cells))
            # print('Num empty cells end  : %s' % str(num_empty_cells_end))
            print('Percentage completed: %0.2f' % perc_completed)
            if (done and is_solved):
                print('Model solved puzzle #: %d' % (i))            
            elif (got_stuck):
                print('Model got stuck on puzzle %d' % (i))
            break

    
test = pd.DataFrame(test_results)
test.to_csv('data/test_last_100_puzzles.csv')
